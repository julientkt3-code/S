...
def fetch_umap_marseille_cameras() -> list[dict]:
    """
    Source : uMap OpenStreetMap - Caméras de vidéo-surveillance à Marseille (map 809)
    Utilise les UUIDs des datalayers (API uMap 3.x).
    """
    MAP_ID = 809
    BASE_URL = "https://umap.openstreetmap.fr"

    # (uuid, slug) — extraits du HTML de la page uMap
    DATALAYERS = [
        ("e6318e78-1ce9-4426-ba69-5a6658e9cd07", "cameras-en-projet"),
        ("ac0e8a3c-2d0e-4cdf-a83d-7daa0eaf9df9", "cameras-actuellement-installees"),
        ("942cc027-c9b6-4872-9081-5d4072192049", "layer-1"),
    ]

    cameras = []
    logger.info("  → uMap Marseille (caméras)...")
    headers = {"User-Agent": "Mozilla/5.0", "Accept": "application/json"}

    for uuid, slug in DATALAYERS:
        urls_to_try = [
            f"{BASE_URL}/fr/datalayer/{MAP_ID}/{uuid}/",
            f"{BASE_URL}/fr/datalayer/{MAP_ID}/{uuid}/0/",
            f"https://umap.openstreetmap.fr/fr/datalayer/9/0/{MAP_ID}/{slug}",
        ]
        fetched = False
        for url in urls_to_try:
            try:
                ...
                for feat in features:
                    ...
                    name = props.get("name") or props.get("nom") or props.get("title") or "Caméra"
                ...
                logger.info(f"    ✅ {slug}: {len(cameras)-count_before} caméras")
                fetched = True
                break
            except Exception:
                continue
        if not fetched:
            logger.warning(f"  ⚠️ Impossible de récupérer le datalayer {slug}")

    logger.info(f"  ✅ {len(cameras)} caméras uMap Marseille au total")
    return cameras
...
def update_cameras():
    ...
    marseille_cams = fetch_umap_marseille_cameras()
    ...
    logger.info(f"✅ {len(all_cams)} caméras sauvegardées ({len(kml_cams)} Paris KML + {len(marseille_cams)} Marseille uMap)")
